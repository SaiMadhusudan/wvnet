...
dataset_path = "./newtrain"
number_e = 3
testset_path = "./val2.csv"
L_rate = 0.0005
batch_size = 1
num_worker_e = 4
...
class DatasetGenerator(Dataset):
    def __init__(self, dir_path, transform=None):
        ...
        transform = T.Compose(
            [
                ...
                T.ToTensor(),
                lambda x: x.float().half(),  # Convert to float16
    ...
    def __getitem__(self, index):
        ...
        random_index1 = np.random.randint(
            0, len(self.group_examples[selected_class]) - 1
        )
        ...
        if index % 2 == 0:
            random_index2 = np.random.randint(
                0, len(self.group_examples[selected_class]) - 1
            )
            ...
            target = torch.tensor([1.0],dtype=int)

        else:
            other_selected_class = np.random.randint(0, len(self.group_examples) - 1)

            while other_selected_class == selected_class:
                other_selected_class = np.random.randint(
                    0, len(self.group_examples) - 1
                )

            random_index2 = np.random.randint(
                0, len(self.group_examples[other_selected_class]) - 1
            )
            index_2 = self.group_examples[other_selected_class][random_index2]
            img2, label2 = self.dataset[index_2]

            target = torch.tensor([0.0],dtype=int)
...
train_dl = DataLoader(
    train_dataset, num_workers=num_worker_e, batch_size=batch_size, shuffle=True
)
val_dl = DataLoader(
    val_dataset, num_workers=num_worker_e, batch_size=batch_size, shuffle=True
)
...
class SiameseNetwork(nn.Module):
    def __init__(self):
        ...
        self.conv2 = nn.Conv2d(96, 128, kernel_size=5)
        self.bn2 = nn.BatchNorm2d(128)
        self.pool2 = nn.MaxPool2d(kernel_size=3, stride=2)
        self.conv3 = nn.Conv2d(128, 128, kernel_size=(1, 3))
        self.conv4 = nn.Conv2d(128, 256, kernel_size=(1, 3))
        self.conv5 = nn.Conv2d(256, 256, kernel_size=(1, 3))
        self.flatten = nn.Flatten()
        self.fc1 = nn.Linear(43008, 10240)
...
def contrastive_loss(output1, output2, label, margin=1.0):
    ...
    loss = torch.mean(
        (1 - label) * torch.pow(similarity, 2)
